...
function DockIcon({ mouseX, children, href, target, label }: any) {
    ...
    let widthSync = useTransform(distance, [-150, 0, 150], [40, 78, 40]);
    ...
    return (
        ...
        >
            {children}
            <span className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-foreground px-2 py-0.5 font-mono text-[10px] uppercase tracking-wide text-background opacity-0 transition-opacity group-hover:opacity-100">
                {label}
            </span>
        </motion.a>
    );
